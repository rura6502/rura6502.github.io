...
Resource Owner의 Credentails를 이용하는 대신에 Client는 access token이라는 것을 이용하게 되는데 이 access token은 scope, lifetime, other access attributes 등으로 구성되어 있는 string이다. Access Token은 Authroization Server로 부터 Client에게 발급되는데 이는 Resource Owner가 승인하여야 한다. 

> 개인 사진(비공개기반)을 보관/인쇄해주는 서비스를 가정할 때, 사용자(end-user, Resource Owner)는 printing service(Client)를 이용할 권한, photo-storing service(Resource Server)에 접근하여 자신이 업로드 해놓은 사진(Protected Resource)에 접근할 권한이 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzMwMTY0MzIwLC0xMjkzNTk5MDEyLDIxND
E1MzY2NTUsNDgzODgyMzQ0LC03MjY1OTM3NzEsMTU4ODk1ODU5
OF19
-->