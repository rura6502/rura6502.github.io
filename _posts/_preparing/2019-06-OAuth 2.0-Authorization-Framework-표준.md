...
(B) 클라이언트는 '리소스 오너가 승인함'을 표현하는 증명서인 '허가 부여(Authorization Grant)'를 받는다. 여기서 허가는 사전정의되어 있는 4가지의 grant type 중 하나이거나 extension grant type을 사용한다. grant type은 클라이언트가 인증을 요청하는데 사용되는 메소드나 인증 서버가 지원하는 타입에 따라서 다른다.
...
'허가 부여(Authorization Grant)'는 '리소스 오너가 제한된 리소스에 접근하는 것을 허용/인증함'을 나타내는 일종의 증명서(Credential)이며 엑세스 토큰을 얻을 때 사용된다.사저
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc2NzQ2NTAzNywxMTA0MTczMTIyLC0zOD
MwMTAyOTYsLTk1OTQ3ODAyMSwtMTUyNDA1NTYwMCwxNjA5NTc2
ODA1LC0xMjkzNTk5MDEyLDIxNDE1MzY2NTUsNDgzODgyMzQ0LC
03MjY1OTM3NzEsMTU4ODk1ODU5OF19
-->