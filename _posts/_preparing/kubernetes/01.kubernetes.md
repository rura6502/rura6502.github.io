...
동일 팟의 모든 컨테이너는 노드의 네트워크를 공유. 모든 노드는 연결되어 있으며 프라이빗 네트워크를 공유. 팟 내에서 실행 되는 컨테이너는 격리된 네트워크에서 실행.

쿠버넷은 IP를 관리하고 DNS를 매핑하여 클러스터 내의 팟들이 서로 통신이 가능하도록 함.

이러한 프라이빗 네트워크에 연결된 팟을 외부로 노출시키기 위해서 '서비스'를 사용. 들어오고 나오

<!--stackedit_data:
eyJoaXN0b3J5IjpbMzkzMjI5MDgsODIyOTg4OTMyLC0yMjM3OT
E4OTQsNjEyMjIwNzg0LDE4NTkzMDE4MDIsMTE3ODk5MzQ5Miwt
NjkxNzE3NDUwLC0xNDYzMDk1NTkwLDY4MTY5Mzg2OF19
-->