...
`kubectl get pods --all-namespaces` 으로 팟을 조회하면 `kube-system` 네임스페이스의 많은 팟들을 확인할 수 있음. 쿠버넷은 자체 시스템 소프트웨어를 셀프 호스팅이 가능하도록 설계되어있음. 쿠버넷을 관리하는 일부 소프트웨어는 외부에서 실행되지만 DNS, 대시보드, 컨트롤러 맨
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNTg0OTYzODYsLTIyMzc5MTg5NCw2MT
