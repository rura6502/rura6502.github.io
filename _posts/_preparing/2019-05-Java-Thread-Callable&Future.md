...
> 예시. 직원1이 직원2에게 할일2를 주고

하지만 ```Callable```을 사용하여 리턴값을 받아서 사용해야 하는 경우엔 리턴값을 반환하는 쓰레드가 끝날 때 까지 기다려야 하고 기다리는 동안 아무것도 못하는 상황이 발생함. 사실상 쓰레드가 제공하는 비동기성을 기대할 수 없음.. 이 부분에서 ```Future```가 이 기다림을 일정부분 유용하게 사용되는데 



<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk2OTQ2MzExOSwtMTAzOTM0MTUwNiwtMT
k1ODA4ODU0Ml19
-->