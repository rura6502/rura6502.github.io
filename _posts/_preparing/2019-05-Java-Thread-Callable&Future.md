...
일반적으로 쓰레드의 경우는 각각 별개로 움직여서 쓰레드간에 시작/종료가 다른 쓰레드에 영향을 미치지 않음. 예를들어 메인쓰레드에서 실행시킨 쓰레드1~10은 각각의 쓰레드가 실행되거나 종료되거나 상관 없이 각자 할일을 마치고 사라지는 형태임. 하지만 ```Callable```을 사용하여 리턴값을 받아야 하면 그 결과를 계산하는 쓰레드가 끝날 때 까지 기다려야 하고 이 기다리는 동안 아무것도 못하는 상황이 발생함. 사실상 쓰레드가 제공하는 비동기성을 기대할 수 없음.. 이 부분에서 ```Future```가 이 기다림을 일정부분 유용하게 사용되는데 



<!--stackedit_data:
eyJoaXN0b3J5IjpbODA2OTM3MzQ4LC0xMDM5MzQxNTA2LC0xOT
U4MDg4NTQyXX0=
-->