...
일반적으로 쓰레드의 경우는 각각 별개로 움직여서 쓰레드간에 시작/종료가 다른 쓰레드에 영향을 미치지 않음. 예를들어 메인쓰레드에서 실행시킨 쓰레드1~10은 각각의 쓰레드가 실행되거나 종료되거나 상관 없이 각자 할일을 마치고 사라지는 형태임. 하지만 ```Callable```을 사용하여 리턴값을 받아야 함



<!--stackedit_data:
eyJoaXN0b3J5IjpbMTEzNDE0NjM5MSwtMTAzOTM0MTUwNiwtMT
k1ODA4ODU0Ml19
-->