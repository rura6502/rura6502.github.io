...
2. 시스템 프로퍼티가 설정되어 있지 않으면 클래스패스상의 `log4j-test.properties` 파일을 찾아서 `log4j.configurationFile` 프로퍼티가 설정되어 있는지 찾음.
3. `log4j.configurationFile` 프로퍼티를 읽는 것에 실패하면 classpath상에서 아래의 파일을 읽는것을 순서대로 시도함.
  1. `log4j2-test.yaml` or `log4j2-test.yml`
  2. `log4j2-test.json` or `log4j2-test.jsn`
  3. `log4j2-test.xml`
4. 테스트 파일(\*-test.\*)을 읽는것에 실패하면 `log4j2.properties` 파일을 읽어서 `log4j.configurationFile` 프로퍼티가 있는지 확인함
5. 만약에 여기에도 시스템 프로퍼티가 없으면 위에 했던 방식데로 파일을 아래의 순서데로 읽는 것을 시도함
	1. `log4j2.yaml` or `log4j2.yml`
	2. `log4j2.json` or `log4j2.jsn`
	3. `log4j2.xml`
6. 이렇게 했는데 없으면 그냥 `DefaultConfiguration` - 콘솔 위주의 출력 을 사용함. 


`DefaultConfiguration`은 아래와 같은 형태로 구성되어 있음
```xml
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzE2Mzg2MDcsMTcwNzY1NjcyNywxNjgyNT
YyMzIwLDE3NTg0NzAyNzAsLTEzOTkxNjc0MDgsLTE3MDc0NTI0
OTYsLTY5MTg1MTM5MiwtNDQ0OTAyNDc4LDEyODAzNjY2OTAsOD
Q2ODE5NTc1LC0xNDAwMjAxNDYzLDIwMzgzNzY1MTFdfQ==
-->