...
3. `log4j.configurationFile` 프로퍼티를 읽는 것에 실패하면 classpath상에서 아래의 파일을 읽는것을 순서대로 시도함.
  ...
  3. `log4j2-test.xml` or `log4j2-test.yml`
  4. `log4j2-test.yaml` or `log4j2-test.yml`
  5. `log4j2-test.yaml` or `log4j2-test.yml`
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NTkxNzI4OTIsMTcwNzY1NjcyNywxNj
