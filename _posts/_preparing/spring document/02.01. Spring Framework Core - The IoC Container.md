...
`org.springframework.context.ApplicationContext` 인터페이스는 스프링 IoC 컨테이너를 대표하고 빈의 인스턴스, 설정, 조립에 대한 책임을 가진다. 컨테이너는 환경설정 메타데이터를 읽어서 객체를 설정하고, 조립하고, 인스턴스화시킨다. 그 환경설정 메타데이터는 XML, Java Annotation, Java Code 등으로 작성될 수 있으며 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI3NDY3NDg0MiwtNzAyMzc3ODgwLDQ1Nj
