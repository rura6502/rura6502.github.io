...
사용자는 일반적으로 스프링이 올바로 동작하기를 믿고, 스프링은 컨테이너를 로드하면서 존재하지 않는 빈을 참조하거나 순환 종속성 등에 대한 문제를 감지한다. 스프링은 빈이 실제로 생성될 때 가능한한 늦게 종속성을 풀고 프로퍼티를 세팅한다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTU5MjQ2MTQyLC0zOTU1NjM3MDUsLTEyNT
