...
> 스프링 팀은 일밙거으로 생성자 주입 방식을 추천한다. 이 방식은 애플리케이션 컴포넌트가 불변객체로써 구현되길 만들어주고 필수적인 디펜던시가 `null`이 아님을 보증한다. 더 나아가 생성자 주입 방식은 사용측에서 항상 완전히 초기화된 상태의 컴포넌트를 반환한다. 추가로, 생성자 주입 방식에서 인자가 많을 경우 안좋게 보일 수 있는데 이는 객체가 너무 많은 책임을 한꺼번에 지고 있다고 의심해볼 수 있으며 리팩토링
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNjYzODg3MzYsLTExNTg5MTc4NSwyMT
...
NjIsOTU0NDM0NTgzLDEyNzc2ODk4MzYsMjE3MzUyMjIwLC01OD
EzMzU2NDYsNDUyMTMyODQ1LDM2NjA5OTcwOSwtMTgyMzk0MTg5
NF19
-->