...
몇몇 `ApplicationContext` 인터페이스의 구현체는 스프링에 의해서 제공된다. 독립형(Stand-alone) 애플리케이션에서 일반적으로 `ClassPathXmlApplicationContext`, `FileSystemXmlApplicationContext`가 많이 사용되며, 전통적인 포맷인 XML
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMjc0OTYzOSw1NzE3MTIxNjksLTcwMj
