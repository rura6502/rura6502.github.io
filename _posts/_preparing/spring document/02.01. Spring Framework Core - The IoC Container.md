...
이후 개발자는 `getBean`을 사용하여 빈의 인스턴스를 얻을 수 있다. `AppliationContext`는 `getBean` 말고도 빈 인스턴스를 얻기 위해 다른 다양한 메소드들을 지원하지만 개발자는 애플리케이션 코드에서 절대 사용하지 않는 것이 좋다. 애플리케이션이 직접 `getBean()`을 호출하지 않음으로써 스프링의 API로 부터 독립적이게 된다. 예를들어 스프링ㅇ
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjE0MjExMTM0MCwtMTgyMzk0MTg5NCwxNj
I1NTgzOTY0LDQ3MzU0MTkzNiwtMTM5MTE1MzIyOCwxMTQ2NTA0
NzI0LC0xMTM1NDY5NDYsMTM0OTg5MDIwMiwtNDEwNDczMjY2LD
MyNTAwMjM4OCwxNTcyMTU5NzU5LC0xODQ5MDAxMTA0LDE3MjE4
NzUwODUsODQ3NTMwMjMyLC05NDgxMDUwMjIsMTE2NjcwODc5NS
wxOTMyNjMxMDM2LDU3MTcxMjE2OSwtNzAyMzc3ODgwLDQ1Njc2
Njk2MV19
-->