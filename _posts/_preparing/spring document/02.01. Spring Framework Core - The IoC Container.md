...
`static factory method` 방식과 유사하게 instance factory 메소드를 사용한 인스턴스화 방식은 새로운 빈을 생성하기 위해 컨테이너로부터 이미 만들어져 있는 빈의 non-static method를 호출한다. 이 방식은 `class` 속성을 비우고 `factory-bean` 속성에 현재 컨테이너에서 사용중인 빈의 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkyMzExMzQwNCwxNDc2Nzk2OTY4LDc0Nz
