...
> 위의 방식으로 파일을 참조할 때 부모 디렉토리를 의미하는 "../"는 동작은 하지만 추천하지 않는다. 이러한 형식의 경로를 사용할 경우 애플리케이션의 외부 종속성을 만들 수 있다. 특히 `classpath:URLs(ex. classpath:../services.xml)도 추천되지 않는다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzMzNDY3OTgyLDEzNDk4OTAyMDIsLTQxMD
