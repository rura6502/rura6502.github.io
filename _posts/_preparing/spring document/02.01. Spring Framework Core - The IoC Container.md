...
이 예제에서 외부 빈 정의는 `services.xml, messageSource.xml, themeSource.xml`에 의해서 로딩된다. 모든 로케이션의 경로는 이 ㅂ정의한 파일의 위치를 기준으로 하는 상대경로로 import되며 이 예제이서 `services.xml`은 이 빈 설정 파일과 같은곳 디렉토리 또는 클래스패스에 위치해 있음을 의미한다. 또한 `messageSource.xml, themeSource.xml`은  반드시
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM5MDgxNTA1NCwxMzQ5ODkwMjAyLC00MT
