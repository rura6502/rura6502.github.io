...
> 스프링 팀은 일밙거으로 생성자 주입 방식을 추천한다. 이 방식은 애플리케이션 컴포넌트가 불변객체로써 구현되길 만들어주고 필수적인 디펜던시가 `null`이 아님을 보증한다. 더 나아가 생성자 주입 방식은 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNTExOTI0NTYsLTExNTg5MTc4NSwyMT
