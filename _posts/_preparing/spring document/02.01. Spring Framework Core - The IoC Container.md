...
`ApplicationContext`는 생성자 주입, 세터 주입 방식을 모두 지원한다. 또한 생성자 주입 방식으로 이미 일부 디펜던시가 주입되고 난 후에도 세터방식을 사용하는 것도 지원한다. 개발자는 `BeanDefinition`에서 이 방식을 지정할 수 
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjAxOTk3MDcsLTExNTg5MTc4NSwyMTUzND
