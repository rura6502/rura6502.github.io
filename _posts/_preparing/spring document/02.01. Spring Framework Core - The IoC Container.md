...
이전 예제에서 서비스 레이어는 `PetStoreServiceImpl` 클래스와 `JpaAccountDao` 타입과 `JpaItemDao` 타입(JPA Object-Relational Mapping standard)의 데이터 엑세스 객체 2개로 이루어져 있다. `property` 엘리먼트의 `name` 속성은 JavaBean 프로퍼티의 이름을 의미하고 `ref` 속성은 다른 빈을 참조한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM5MzI4NzE4OCwxNTcyMTU5NzU5LC0xOD
