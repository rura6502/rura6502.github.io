...
Dependency Injection(이하 DI)는 생성자 주입 방식, 팩토리 메서드 방식, 팩토리 메서드로부터 반환되거나 구성되어 생성되는 객체 인스턴스로 만들어진 프로퍼티 등으로 디펜던시(작업하는 다른 객체)를 정의할 수 있는 프로세스이다. 컨테이너는 이런 디펜던시들을 빈이 생성될때 주입한다. 이런 프로세스는 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg1OTA2ODE3NCwtNTQwNzA2NjQzLDk1OD
