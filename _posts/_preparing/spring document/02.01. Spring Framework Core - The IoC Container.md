...
일반적인 엔터프라이즈 애플리케이션은 단일 오브젝트로 이루어지지 않았으며 심지어 매우 간단한 애플리케이션도 몇몇의 오브젝드들이 함께 연계되어 엔드유저에게 정보를 서비스를 제공한다. 이번 섹션은 다수의 빈 정의를 정의하는 방법 부터 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAyNDA0OTgxNiwtNTQwNzA2NjQzLDk1OD
