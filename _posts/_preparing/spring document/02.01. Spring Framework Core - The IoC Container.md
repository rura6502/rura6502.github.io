...
스프링 컨테이너는 컨테이너에서 생성된 각각의 빈 설정을 검증한다. 그러나 빈 프로퍼티는 실제로 빈이 생성되기 전까지 세팅되지 스스로 세팅되지 않는다. 컨테이너가 생성될 때 싱글톤 빈, 사전 인스턴스화 되도록 설정된 빈이 생성된다. 빈의 범위는 `Bean Scopes`에 의해서 정의된다. 별도로 설정하지 않으면 빈은 리퀘스트가 일어날 때 생성된다. 잠재적으로 빈의 생성은 빈에서 필요로 하는 디펜던시들이 생성되고 주입되어 서로 엮이는 일종의 그래프 형태를 이룬다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY4NTU3NzMzOSw0NTY5OTUyNjksLTEwMD
AxNDQ0MjksLTM1ODYzOTM4NCwtNDYxMTM3MTMwLDE0MzczMDI3
MSwtMTE1ODkxNzg1LDIxNTM0MDEzNiwxNjM2NDUyOTEwLC01ND
A3MDY2NDMsOTU4OTMxODI3LDE0NzY3OTY5NjgsNzQ3MDQ4ODMx
LDE2MjI4ODAyMjgsLTUwOTY0ODYwNCwtNDY5MDM0NzkzLC0xOT
c4MDAzMTcsLTUyOTE5ODQ2Miw5NTQ0MzQ1ODMsMTI3NzY4OTgz
Nl19
-->