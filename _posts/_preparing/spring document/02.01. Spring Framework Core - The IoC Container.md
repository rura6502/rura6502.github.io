...
`ApplicationContext`의 구현체는 특정 빈을 어떻게 생성할 것인지에 대한 정보를 포함하는 빈 정의 외에도 개발자가 컨테이너 외부에서 이미 생성되어져 있는 객체의 등록도 허용한다. `ApplicationContext`의 `BeanFactory`에 있는 `getBeanFactory()`를



<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgzOTk0NDQ3Myw0NTIxMzI4NDUsMzY2MD
