...
사용자는 일반적으로 스프링이 올바로 동작하기를 믿고, 스프링은 컨테이너를 로드하면서 존재하지 않는 빈을 참조하거나 순환 종속성 등에 대한 문제를 감지한다. 스프링은 빈이 실제로 생성될 때 가능한한 늦게 종속성을 풀고 프로퍼티를 세팅한다. 이것은 사용자가 요청한 오브젝트가 종속성의 문제로 생성에 문제가 있을 경우, 올바르게 세팅된 스프링 컨테이너만이 익셉션을 나중에 발생시킬 수 있다는 것을 의미한다. 예를들어 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMjE5MTA3MywtMzk1NTYzNzA1LC0xMj
