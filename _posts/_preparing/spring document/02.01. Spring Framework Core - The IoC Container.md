...
사용자가 `name` 또는 `id`를 선언하지 않으면 컨테이너에서는 새로운 고유 이름을 생성하여 배정한다. 그러나 `ref` 또는 `Service Locator Style lookup`을 사용하여 이름으로 빈을 참조하려는 경우 바
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMzY4OTYyOSwyMTczNTIyMjAsLTU4MT
