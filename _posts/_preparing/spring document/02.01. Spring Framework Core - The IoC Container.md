...
Dependency Injection(이하 DI)는 생성자 주입 방식, 팩토리 메서드 방식, 팩토리 메서드로부터 반환되거나 구성되어 생성되는 객체 인스턴스로 만들어진 프로퍼티 등으로 디펜던시(작업하는 달)를 정의할 수 있는 프로세스이다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk2NDIzNTE4OCwtNTQwNzA2NjQzLDk1OD
