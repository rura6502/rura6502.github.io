...
`ApplicationContext`의 구현체는 특정 빈을 어떻게 생성할 것인지에 대한 정보를 포함하는 빈 정의 외에도 개발자가 컨테이너 외부에서 이미 생성되어져 있는 객체의 등록도 허용한다. `ApplicationContext`의 `BeanFactory`에 있는 `getBeanFactory()`를 통하여 `DefaultListableBeanFactory`를 얻을 수 있으며 이것을 통하여 `registerSingleton(..)` 또는 `registerBeanDefinition(..)`를 호출하여 빈을 등록할 수 있다. 그러나 일반적으로 애플리케이션에서 



<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQwNzUzOTI5OSw0NTIxMzI4NDUsMzY2MD
k5NzA5LC0xODIzOTQxODk0LDE2MjU1ODM5NjQsNDczNTQxOTM2
LC0xMzkxMTUzMjI4LDExNDY1MDQ3MjQsLTExMzU0Njk0NiwxMz
Q5ODkwMjAyLC00MTA0NzMyNjYsMzI1MDAyMzg4LDE1NzIxNTk3
NTksLTE4NDkwMDExMDQsMTcyMTg3NTA4NSw4NDc1MzAyMzIsLT
k0ODEwNTAyMiwxMTY2NzA4Nzk1LDE5MzI2MzEwMzYsNTcxNzEy
MTY5XX0=
-->