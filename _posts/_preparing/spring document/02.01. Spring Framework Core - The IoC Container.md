...
> 스프링 팀은 일밙거으로 생성자 주입 방식을 추천한다. 이 방식은 애플리케이션 컴포넌트가 불변객체로써 구현되길 만들어주고 필수적인 디펜던시가 `null`이 아님을 보증한다. 더 나아가 생성자 주입 방식은 사용측에서 항상 완전히 초기화된 상태의 컴포넌트를 반환한다. 추가로, 생성자 주입 방식에서 인자가 많을 경우 안좋게 보일 수 있는데 이는 객체가 너무 많은 책임을 한꺼번에 지고 있다고 의심해볼 수 있으며 리팩토링을 해야함을 의미할 수 있다.
> 세터 주입 방식은 주로 선택적인 클래스 내에서 적절한 기본값을 설정할 수 있는 디펜던시를 주입할 때 사용된다. 그렇지 않으면 디펜던시를 사용하는 모든 코드에 not-null-check를 반드시 해야한다. 세터 인젝선의 이점은 생성되고 난 후에도 추후에 re-injection, reconfiguration이 가능하다는 것이다. `JMX MBeans`를 통한 매니지먼트는 

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgwMzE5MzM3OCwtMTE1ODkxNzg1LDIxNT
M0MDEzNiwxNjM2NDUyOTEwLC01NDA3MDY2NDMsOTU4OTMxODI3
LDE0NzY3OTY5NjgsNzQ3MDQ4ODMxLDE2MjI4ODAyMjgsLTUwOT
Y0ODYwNCwtNDY5MDM0NzkzLC0xOTc4MDAzMTcsLTUyOTE5ODQ2
Miw5NTQ0MzQ1ODMsMTI3NzY4OTgzNiwyMTczNTIyMjAsLTU4MT
MzNTY0Niw0NTIxMzI4NDUsMzY2MDk5NzA5LC0xODIzOTQxODk0
XX0=
-->