...
이전 예제에서 서비스 레이어는 `PetStoreServiceImpl` 클래스와 `JpaAccountDao` 타입과 `JpaItemDao` 타입(JPA Object-Relational Mapping standard)의 데이터 엑세스 객체 2개로 이루어져 있다. `property` 엘리먼트의 `name` 속성은 JavaBean 프로퍼티의 이름을 의미하고 `ref` 속성은 참조 하는 다른 빈의 이름을 의미한다.  `id`와 `ref`는 서로같에 협업하는 개체들간의 으
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYwMzI1Njk1MSwxNTcyMTU5NzU5LC0xOD
Q5MDAxMTA0LDE3MjE4NzUwODUsODQ3NTMwMjMyLC05NDgxMDUw
MjIsMTE2NjcwODc5NSwxOTMyNjMxMDM2LDU3MTcxMjE2OSwtNz
AyMzc3ODgwLDQ1Njc2Njk2MSwtMTUwMzk4ODk0NywxMzg5MTU0
ODUwXX0=
-->