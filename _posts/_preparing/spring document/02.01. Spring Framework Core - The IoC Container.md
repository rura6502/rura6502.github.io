...
이 예제에서 외부 빈 정의는 `services.xml, messageSource.xml, themeSource.xml`에 의해서 로딩된다. 모든 로케이션의 경로는 정의한 파일을 import한 위치를 기준으로 하는 상대경로로 import되며 이 예제이서 `services.xml`은 이 빈 설정 파일과 같은곳 디렉토리 또는 클래스패스에 위치해 있음을 의미한다. 또한 `messageSource.xml, themeSource.xml`은  반드시 import한 위치의 `resources` 폴더 안에 있어야 한다. 여기서 맨 앞에 나오는 `/`는 무시된다. 그러나 여기서 사용되는 경로가 상대경로 일 경우 맨 앞에 오는 `/`는 쓰지 않는 것이 좋다. `<beans/>` 엘리먼트를 포함하고 있는 import되어져 있는 파일의 내용은  XML bean definition 문법에 맞
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU0OTIyNjcwOSwxMzQ5ODkwMjAyLC00MT
A0NzMyNjYsMzI1MDAyMzg4LDE1NzIxNTk3NTksLTE4NDkwMDEx
MDQsMTcyMTg3NTA4NSw4NDc1MzAyMzIsLTk0ODEwNTAyMiwxMT
Y2NzA4Nzk1LDE5MzI2MzEwMzYsNTcxNzEyMTY5LC03MDIzNzc4
ODAsNDU2NzY2OTYxLC0xNTAzOTg4OTQ3LDEzODkxNTQ4NTBdfQ
==
-->