...
대부분의 애플레케이션 시나리오에서 개발자가 코드로 하나 또는 그 이상의 스프링 IoC 컨테이너를 인스턴스화 시킬 필요는 없다. 예를들어 웹 애플리케이션을 설정할때 간단한 8줄의 boilerpalte web descriptor XML 구문을 `web.xml` 파일에 추가하면 애플리케이션을 구성하는데 충분하다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTA2ODI4NjcxLDE5MzI2MzEwMzYsNTcxNz
EyMTY5LC03MDIzNzc4ODAsNDU2NzY2OTYxLC0xNTAzOTg4OTQ3
LDEzODkxNTQ4NTBdfQ==
-->