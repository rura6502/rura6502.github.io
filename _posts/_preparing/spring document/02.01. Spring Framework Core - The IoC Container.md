...
이후 개발자는 `getBean`을 사용하여 빈의 인스턴스를 얻을 수 있다. `AppliationContext`는 `getBean` 말고도 빈 인스턴스를 얻기 위해 다른 다양한 메소드들을 지원하지만 개발자는 애플리케이션 코드에서 절대 사용하지 않는 것이 좋다. 사실 애플리케이션이 직접 `getBean()`을 호출하여 빈을 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4MjM5NDE4OTQsMTYyNTU4Mzk2NCw0Nz
M1NDE5MzYsLTEzOTExNTMyMjgsMTE0NjUwNDcyNCwtMTEzNTQ2
OTQ2LDEzNDk4OTAyMDIsLTQxMDQ3MzI2NiwzMjUwMDIzODgsMT
U3MjE1OTc1OSwtMTg0OTAwMTEwNCwxNzIxODc1MDg1LDg0NzUz
MDIzMiwtOTQ4MTA1MDIyLDExNjY3MDg3OTUsMTkzMjYzMTAzNi
w1NzE3MTIxNjksLTcwMjM3Nzg4MCw0NTY3NjY5NjEsLTE1MDM5
ODg5NDddfQ==
-->