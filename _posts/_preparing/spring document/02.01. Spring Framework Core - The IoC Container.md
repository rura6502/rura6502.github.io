...
이 예제에서 외부 빈 정의는 `services.xml, messageSource.xml, themeSource.xml`에 의해서 로딩된다. 모든 로케이션의 경로는 정의한 파일을 import한 위치를 기준으로 하는 상대경로로 import되며 이 예제이서 `services.xml`은 이 빈 설정 파일과 같은곳 디렉토리 또는 클래스패스에 위치해 있음을 의미한다. 또한 `messageSource.xml, themeSource.xml`은  반드시 import한 위치의 `resources` 폴더 안에 있어야 한다. 여기서 맨 앞에 나오는 `/`는 무시된다. 그러나 상대경로
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI5NTQ3NTAwLDEzNDk4OTAyMDIsLTQxMD
Q3MzI2NiwzMjUwMDIzODgsMTU3MjE1OTc1OSwtMTg0OTAwMTEw
NCwxNzIxODc1MDg1LDg0NzUzMDIzMiwtOTQ4MTA1MDIyLDExNj
Y3MDg3OTUsMTkzMjYzMTAzNiw1NzE3MTIxNjksLTcwMjM3Nzg4
MCw0NTY3NjY5NjEsLTE1MDM5ODg5NDcsMTM4OTE1NDg1MF19
-->