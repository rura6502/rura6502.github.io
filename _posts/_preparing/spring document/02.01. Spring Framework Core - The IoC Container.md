...
> 만약 생성자 주입방식을 사용한다면 순환 종속성을 방지할 수 잇다. 예를들어 `ClassA`가 `ClassB`의 인스턴스를 생성자 주입 방식으로 주입하고 `ClassB`가 `ClassA`의 인스턴스를 생성자 주입방식으로 받는다고 하자. 만약 `ClassA`, `ClassB`를 빈 설정을 하면 스프링 IoC 컨테이너는 런타임 환경을 구성할 때 순환 참조 문제를 발견하고 `BeanCurrentlyInCreationException`을 발생시킬 것이다.
> 하나의 
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDg5NjMwMDYsLTEyNTE2ODE2MDAsNDU2OT
k1MjY5LC0xMDAwMTQ0NDI5LC0zNTg2MzkzODQsLTQ2MTEzNzEz
MCwxNDM3MzAyNzEsLTExNTg5MTc4NSwyMTUzNDAxMzYsMTYzNj
Q1MjkxMCwtNTQwNzA2NjQzLDk1ODkzMTgyNywxNDc2Nzk2OTY4
LDc0NzA0ODgzMSwxNjIyODgwMjI4LC01MDk2NDg2MDQsLTQ2OT
AzNDc5MywtMTk3ODAwMzE3LC01MjkxOTg0NjIsOTU0NDM0NTgz
XX0=
-->