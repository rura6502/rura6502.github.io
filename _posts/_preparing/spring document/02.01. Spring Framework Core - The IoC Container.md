...
XML 기반의 환경설정 메타데이터에서 `id`과 `name` 속성을 사용하여 빈을 구분할 수 있으며 둘 다 쓸수도 있다. 아이디 속성은 하나만 선언할 수 있으며 관례적으로 영문과 숫자로 이루어져 myBean, someService 와 같은 형태로 네이밍 한다. 특수문자도 쓸 수 있다. `name` 속성의 경우 콤마( , ), 세미콜론( ; ), 공백( )을 사용하여 별칭을 구분하여 선언할 수 있다.  As a historical note, in versions prior to Spring 3.1, the `id` attribute was defined as an `xsd:ID` type, which constrained possible characters. As of 3.1, it is defined as an `xsd:string` type. bean의 `id` 속성은 컨테이너 안에서 유일해야하는 귳



<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYyNjE3NjI5MiwyMTczNTIyMjAsLTU4MT
MzNTY0Niw0NTIxMzI4NDUsMzY2MDk5NzA5LC0xODIzOTQxODk0
LDE2MjU1ODM5NjQsNDczNTQxOTM2LC0xMzkxMTUzMjI4LDExND
Y1MDQ3MjQsLTExMzU0Njk0NiwxMzQ5ODkwMjAyLC00MTA0NzMy
NjYsMzI1MDAyMzg4LDE1NzIxNTk3NTksLTE4NDkwMDExMDQsMT
cyMTg3NTA4NSw4NDc1MzAyMzIsLTk0ODEwNTAyMiwxMTY2NzA4
Nzk1XX0=
-->