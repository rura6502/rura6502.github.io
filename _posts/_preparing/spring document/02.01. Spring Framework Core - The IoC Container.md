...
만약 사용자가 XML 기반의 환경설정 메타데이터를 사용하였다면 `<bean/>` 엘리먼트의 class` 속성에 사용자는 초기화할 객체의 타입(또는 클래스)를 나열한다. `class` 속성(내부적으로 `BeanDefinition 인스턴스의 `Class` 프로퍼티)으

<!--stackedit_data:
eyJoaXN0b3J5IjpbOTc1MjUxMjA2LDk1NDQzNDU4MywxMjc3Nj
