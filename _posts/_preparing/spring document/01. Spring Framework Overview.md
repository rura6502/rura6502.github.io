...
현재 스프링 프레임워크는 다양한 모듈로 분리될 수 있으며 애플리케이션의 요구에 따라서 필요한 모듈들을 선택하여 사용할 수 있다. 이 중 핵심이 되는 모듈은 의존성 주입(Dependency injection) 메커니즘과 환경 설정 모델(Configuration Model)을 포함하고 있는 코어 컨테이너(Core Container)이다. 이를 시작으로 메세징, 트렌젝션 데이터, 영속성(Persistence), 웹등 애플리케이션을 만들기 위한 다양한 기초를 만들어 준다.
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUwMjMyMzQ5NiwxMTYyNTcxNTEyLC0xND
QxOTExODk1LDEwNjMxNzYwNjJdfQ==
-->