...
```
  ...
  ```synchronized``` 블록을 사용하여 쓰레드가 공유 변수에 접근하여 변경하고 출력하기까지의 과정에 lock(자물쇄)를 걸어 어떤 쓰레드가 ```synchronized``` 안의 블록을 실행하는 동안, 다른 쓰레드는 해당 블록을 수행하지 않고 종료될 때 까지 기다렸다가 수행되도록 하였다. 이렇게 하여 특정 쓰레드가 실행 되는 동안 다른 쓰레드가 공유 변수에 접근하여 값을 변경하는 예측할 수 없는 상황을 방지할 수있다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbODQ2NTkxNzA3LC0xMjE0MDYzODUsLTMwNT
