...
```
이는 `TodoAPI`클래스가 `TodoCSVReader`클래스와 직접 연결되어 만약 Todo데이터를 제공하는 리소스가 CSV에서 데이터베이스 또는 엑셀의 xlsx로 제공하고자

 Todo를 읽어오는 역할을 하는 클래스가 변경될 경우 TodoAPI를 제공하는 역할을 가진 `TodoAPI`클래스가 같이 변경되어야 함을 의미한다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3MzQxNzMzMjcsLTEwNDY3NDg0NjgsLT
