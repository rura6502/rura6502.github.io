...
클래스도 동일하다. 규격만 정해져 있고 zmf해당 규격을 사용한다면, 클래스A가 클래스B를 사용할 때 굳이 클래스B가 내부적으로는 어떻게 동작하는지는 클래스A가 신경쓸 필요가 없고 클래스B가 내부적으로 변한다고 해도 규격은 약속되어 있으니 변화 없이 사용할 수 있다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAzODI5MzY0Niw4MDIzNDI0MDIsLTE5MD
