...
## Decoupling
결합도(coupling) 높다는 뜻은 어떤 클래스가 동작하기 위해서 특정 클래스와 그 구현에 의존성(얼마나 깊이 참조했는가)이 높다는 것을 의미함. 결합도가 높을 경우 특정 클래스의 변화가 다른 클래스에 영향을 많이 미칠 수 있어 변화에 취약하다는 것을 의미함.

건전지를 사용할 때 우리는 에너자이저
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMjU1MzA0MDYsOTAzMzcyODg0LC04MD
k5MDYzNjZdfQ==
-->